...
func result1() :
	...
	var text : String = "With your increased ability to breathe freely, you search the entire pool and notice many large bones of bipeds littering the bottom.  This pool has proved to be a watery grave for more than one unlucky adventurer."
	await ScriptHelperFuncsClass.display_text_wait_noise(text,'message nod.wav')
	await ScriptHelperFuncsClass.play_sound_divinity(666)
	text = "You move through an underground tunnel and emerge into a cave that is totally immersed in a strange, green light.  The light comes from a strange moss-type plant that covers the walls forming a thick mat."
	await ScriptHelperFuncsClass.display_text_wait_noise(text,'message nod.wav')
	#New function
	await teleport_to_location(3, 18, 61, 667)
