const classrace_name : String = "Archer"
...
static func _level_up(_character, _new_level : int) :
	...
	# ADD APR AT LEVEL
	if [5,11,17,23,29,35,41,47].has(_new_level) :
...
static func _character_creation_gifts(_character) :
	...
	resources.load_item_resources(Paths.realmzfolderpath+"shared_assets/items/")
