const classrace_name : String = "Battle Mage"
const classrace_types : Array = ["Mage Classes"]  # leave it as is
...
static func _level_up(_character, _new_level : int) :
	...
	# ADD APR AT LEVEL
	if [7,13,19,25,31].has(_new_level) :
...
static func can_learn_spell(_character, _spell) -> int :
	...
	if _spell.has("school_levels") and not _spell.school_levels.is_empty():
		...
		if _spell.school_levels.has("Sorcerer"):
			if _spell.school_levels["Sorcerer"] <= 5:
				return _spell.school_levels["Sorcerer"]
			else:
				return 10  # Can't learn Sorcerer spells above level 5
...
static func _character_creation_gifts(_character) :
	...
	for name in ["Scimitar","Dagger","Helm","Chain Gloves","Steel Shod Boots","Shield"] :
