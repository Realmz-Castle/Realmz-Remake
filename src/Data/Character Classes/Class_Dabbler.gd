const classrace_name : String = "Dabbler"
...
static func _level_up(_character, _new_level : int) :
	...
	# ADD APR AT LEVEL
	if [7,14,21,28,35,42].has(_new_level) :
...
static func can_learn_spell(_character, _spell) -> int :
	...
	if _spell.has("school_levels") and not _spell.school_levels.is_empty():
		...
		if _spell.school_levels.has("Enchanter"):
			if _spell.school_levels["Enchanter"] <= 5:
...
static func _character_creation_gifts(_character) :
	...
	for name in ["Scimitar","Dagger","Chain Armor","Helm","Chain Gloves","Steel Shod Boots","Shield"] :
