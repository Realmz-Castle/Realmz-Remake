const classrace_name : String = "Marksman"
const classrace_types : Array = ["Warrior Classes"]  # leave it as is
...
const base_stat_bonuses : Dictionary = {
	...
	"MultiplierMental" : 0.9875, 
	"Melee_Crit_Rate" : 0.04,	
	"Melee_Crit_Mult" : 2.0,	
	"Ranged_Crit_Rate" : 0.04,   
...
static func _level_up(_character, _new_level : int) :
	...
	# ADD APR AT LEVEL
	if [7,14,21,28,35,42,49,56].has(_new_level) :
...
static func _character_creation_gifts(_character) :
	...
	for name in ["Nunchuka","Bow","Quiver of Arrows","Leather Armor","Leather Gloves","Leather Boots"] :
		var item = resources.create_item_instance(name)
		_character.add_inventory_item(item)
	_character.money[0] += 100
