const classrace_name : String = "Crusader"
...
static func _character_creation_gifts(_character) :
	...
	_character.spells = [[],[],[],[]]
	for name in ["Longsword","Dagger","Chain Armor","Helm","Chain Gloves","Steel Shod Boots","Shield"] :
		var item = resources.create_item_instance(name)
		_character.add_inventory_item(item)
	_character.money[0] += 5
