...
class GodotModPlayerInstrument:
	...
	var array_ass:Array		# AudioStreamWAV[]
...
class GodotModPlayerChannelStatus:
	var volume_table:PackedFloat32Array = PackedFloat32Array([
...
# ファイル
@export var file:String = "" : set = set_file #(String, FILE, "*.mod,*.xm") 
# 再生中か？
@export var playing:bool = false #(bool)
# 音量
@export var volume_db:float = -20.0 : set = set_volume_db #(float, -144.0, 0.0)
# キーシフト（未実装）
#export (int) var key_shift:int = 0
# ループフラグ
@export var loop:bool = false #(bool) 
...
@export var bus:String = "Master"  # (String)
...
# Modデータ
var mod_data:Mod.Mod = null : set = set_mod_data
# テンポ
