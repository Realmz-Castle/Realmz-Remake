...
| Encounters, services, and time | Partial | Simple, complex, and rogue results; action, spoken, spell, scroll, item, trap, and door responses; chance-based rogue lock controls; shops, temples, banks, and timed-encounter scheduling. | Remake intentionally uses a native chance roll instead of Classic's timed tumbler minigame. Some built-in warnings and optional modal pacing remain fidelity work. Every referenced resource must resolve exactly. |
