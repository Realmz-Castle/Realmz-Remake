...
| Unimplemented encounter-result actions | Partial | Source-backed | After Take Gold, Give Condition, and Selective Combat, the full City of Bywater audit has two executable Data ED actions without a handler, both Alter Time Encounter (`54`). The interpreter reports each exact record and slot instead of skipping it. Remake has a native clock, but the Classic encounter-time contract is not bridged yet. | Alter Time Encounter preserves its mutation and continuation behavior in focused fixtures, and a full-bundle audit leaves no executable unknowns. |
