...
| Battles and battle context | Native, unwired | Source-backed | Battle requests stop at the adapter. Opcode `127` can query the native live roster by preserved Classic monster ID and stop its combat macro when the required monster is absent. Opcodes `121` and `125` remove eligible live combatants through the native roster, preserve Classic's undead-tier and faction rules, and register hostile removals with battle rewards. Opcode `123` filters its compiled monster IDs to the macro actor's faction and applies Remake's permanent fleeing AI; queued and on-death macros will need to supply their actor faction when the battle bridge invokes them, and the current retreat AI does not remove an enemy at the battlefield edge. Opcode `100` can end a live native battle as a forced victory, use the normal cleanup flow with experience-only rewards, and terminate its combat macro; resuming the suspended outer action list at Classic slot 8 still depends on the battle bridge. Other combat-only actions and battle macros do not yet have the Classic battle context they require. Opcodes `82` and `83` persist priest-turning availability and expose it to each battle request, but Remake has no turn-undead combat action to consume that gate. | Authored battles can start, respect priest-turning availability, return victory or cowardice, execute combat action points and macros, and resume the suspended Classic action list. |
...
The checked City of Bywater bundle currently has 26 active action slots without
...
| --- | --- | ---: | --- |
| Combat and battle macros | `124`, `126` | 26 | Native combat and Classic battle context |
