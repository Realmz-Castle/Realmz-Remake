...
| Unimplemented encounter-result actions | Partial | Source-backed | After Take Gold, the full City of Bywater audit has eight executable Data ED actions without handlers: Give Condition (`43`), Selective Combat (`48`), and Alter Time Encounter (`54`). The interpreter reports every exact record and slot instead of skipping them. Remake has relevant native character-state, combat, and clock systems, but the Classic branching contracts are not bridged yet. | Each action preserves its Classic selection, mutation, result branch, and continuation behavior in focused fixtures and a full-bundle audit leaves no executable unknowns. |
