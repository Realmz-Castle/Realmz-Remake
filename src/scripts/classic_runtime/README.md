...
Against the checked City of Bywater compatibility baseline, these handlers cover 2,229 of 2,734 active action slots. Another 470 slots are skipped only because the bundle's source-backed dispatcher evidence identifies them as Realmz no-ops. Together, the proof of concept has defined behavior for 2,699 slots, or 98.7% of active slots. This is a semantic coverage measurement, not a playability percentage. Native command adapters, resource bridges, and 35 action slots across additional opcodes remain. Opcodes `35`, `42`, and `44` also occur inside encounter results and those uses are not reflected in this trigger-slot count.
