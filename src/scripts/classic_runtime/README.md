...
Learned spell persistence still uses the native resource name as its primary restore key. That is insufficient when two exact Classic records share a display name but differ mechanically. ISY-426 tracks adding stable Classic identity to learned spell entries, conservative migration of legacy name-only saves, and exact-ID save/load coverage. Until then, the support matrix may prove an exact spell resource without claiming learned-spell integration for an ambiguous same-name variant.
