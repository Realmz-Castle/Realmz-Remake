...
Against the current Providence export of City of Bywater, these handlers cover 2,045 of 2,734 active action slots. Another 470 slots are skipped only because the bundle's source-backed dispatcher evidence identifies them as Realmz no-ops. Together, the proof of concept has defined behavior for 2,515 slots, or 92.0% of active slots. This is a semantic coverage measurement, not a playability percentage. Native command adapters and 219 action slots across additional opcodes remain. Opcodes `35` and `44` occur inside encounter results and therefore are not reflected in this trigger-slot count.
