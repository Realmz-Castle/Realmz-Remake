...
Compiled campaigns installed under `src/Campaigns/<campaign>` now use the normal campaign panel. The installer materializes missing outdoor native maps from complete normalized tile arrays while the package is staged, including stable Action Point IDs, trigger chances, and random rectangles. Raw custom atlases and special negative tiles are not flattened into stock terrain; they remain explicit installation blockers until decoded native resources are present. The selector displays the manifest title, format version, compatibility profile, and readiness state. A structurally valid bundle remains visible but is blocked with an actionable diagnostic when semantic readiness fails or its native start-map files are incomplete. Starting a ready campaign through the normal party flow loads native resources, creates the compatibility session, and enters the compiled start location.
