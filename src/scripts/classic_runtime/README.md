...
Opcode `6` converts Classic's five fixed 200-slot stock categories into Remake's native shop categories and applies the authored inflation to purchase and resale prices. A positive shop ID makes the Shop control available until movement; a negative ID opens it immediately. Existing native shop state is retained when the same shop is loaded again so purchased stock survives Remake's save lifecycle. Every stocked item must resolve to a loaded Remake resource. The current full City of Bywater bundle does not include names for its scenario-specific items, so affected shops remain an explicit resource boundary rather than silently omitting their merchandise.
...
Against the checked City of Bywater compatibility baseline, these handlers cover 2,194 of 2,734 active action slots. Another 470 slots are skipped only because the bundle's source-backed dispatcher evidence identifies them as Realmz no-ops. Together, the proof of concept has defined behavior for 2,664 slots, or 97.4% of active slots. This is a semantic coverage measurement, not a playability percentage. Native command adapters and 70 action slots across additional opcodes remain. Opcodes `35`, `42`, and `44` also occur inside encounter results and those uses are not reflected in this trigger-slot count.
