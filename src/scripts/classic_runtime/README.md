...
Normal profile saves now include a versioned Classic envelope. It records the compiled campaign identity, `ClassicRuntimeState` snapshot, and adapter-owned equipment capture; load validates the version and identity before changing game state. Older saves without the envelope retain their native map and coordinates with fresh compatibility defaults, while a save from a newer unsupported schema is left untouched and reported in the save panel. Saving and restoring duplicate runtime state without modifying the compiled campaign bundle.
